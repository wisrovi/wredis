...
## Description
Demonstrates using the async_cache decorator to cache async function results. The first call executes the function, subsequent calls return cached results.
...
```mermaid
graph LR
    A[Async Function] --> B[async_cache Decorator]
    B --> C[Redis Cache]
```
