...
large_list = [
    {"id": i, "value": f"item_{i}", "active": i % 2 == 0} for i in range(10000)
]
...
print(
    f"Serialized size: {len(serialized)} characters ({len(serialized) / 1024:.1f} KB)"
)
...
large_dict = {
    f"key_{i:04d}": {"value": i * 1.5, "type": "numeric"} for i in range(5000)
}
